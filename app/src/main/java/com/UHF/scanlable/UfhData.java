...
public class UfhData {
	...
	static Map<String, Long> scanResult6b = new HashMap<String, Long>();
	...
	public static void read6c(int selectedEd,int TidFlag){
		try{
			...
			if((selectedEd==0)||(selectedEd==1))
				target=0;
			String[] lable = UhfGetData.Scan6C(selectedEd,TidFlag,target);
			if(lable == null){ 
				scaned_num = 0;
				cmdnum=0;
				cmdsl = 0;
	...
	public static void read6b(){
		...
		for (int i = 0; i < scaned_num; i++) {
			...
			scanResult6b.put(key, Long.valueOf(RSSI,16));
		}
	}
...
//		private static final String "" = null;
		...
		public static String[] Scan6C(int Session,int TidFlag,byte target)
		{	
			try{
				int result=0x30;
				if(TidFlag==0)
				{
					result = uhf.EPCC1G2_ScanEPC((byte)0x06, (byte)Session,(byte)(0),(byte)(0),target);
				}
				else if(TidFlag==1)
				{
					result = uhf.EPCC1G2_ScanEPC((byte)0x06, (byte)Session,(byte)(0),(byte)(6),target);
				}
				SoundFlag=false;
				if(result==0){
					SoundFlag=true;
